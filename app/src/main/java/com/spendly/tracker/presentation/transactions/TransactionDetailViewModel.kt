...
import com.spendly.tracker.data.repository.BudgetGroupRepository
...
import com.spendly.tracker.domain.model.TransactionRenameCandidate
...
import com.spendly.tracker.domain.usecase.UpdateTransactionRequest
import com.spendly.tracker.domain.usecase.UpdateTransactionUseCase
import dagger.hilt.android.lifecycle.HiltViewModel
import com.spendly.tracker.utils.MerchantAliasAuditor
import com.spendly.tracker.utils.MerchantNameMatcher
import com.spendly.tracker.utils.SmsMerchantAliasHints
...
class TransactionDetailViewModel @Inject constructor(
    ...
    private val _showAmountError = MutableStateFlow(false)
    val showAmountError: StateFlow<Boolean> = _showAmountError.asStateFlow()

    private val _showMerchantError = MutableStateFlow(false)
    val showMerchantError: StateFlow<Boolean> = _showMerchantError.asStateFlow()
    ...
    private suspend fun determinePrimaryCurrency(transaction: TransactionEntity) {
        ...
        val primaryCurrency = if (isUnified) {
            ...
            if (!bankName.isNullOrEmpty()) {
                com.spendly.tracker.utils.CurrencyFormatter.getBankBaseCurrency(bankName)
            } else {
    ...
    fun updateMerchantName(name: String) {
        _showMerchantError.value = false
            ...
            _editableTransaction.update { current ->
                ...
                val newTransferKind = if (type == TransactionType.TRANSFER)
                    current?.transferKind?.takeIf {
                        it == com.spendly.tracker.data.database.entity.TransferKind.SELF_TRANSFER ||
                        it == com.spendly.tracker.data.database.entity.TransferKind.OTHERS_TRANSFER ||
                        it == com.spendly.tracker.data.database.entity.TransferKind.CC_BILL_PAYMENT
                    } ?: com.spendly.tracker.data.database.entity.TransferKind.SELF_TRANSFER
                else null
