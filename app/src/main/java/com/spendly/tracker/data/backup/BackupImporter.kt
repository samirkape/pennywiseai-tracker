...
class BackupImporter @Inject constructor(
    ...
    private suspend fun importPreferences(preferences: PreferencesSnapshot) {
        ...
        preferences.theme.themeStyle?.let { name ->
            runCatching { com.spendly.tracker.data.preferences.ThemeStyle.valueOf(name) }
                .getOrNull()?.let { userPreferencesRepository.updateThemeStyle(it) }
        }
        preferences.theme.accentColor?.let { name ->
            runCatching { com.spendly.tracker.data.preferences.AccentColor.valueOf(name) }
        ...
        preferences.theme.appFont?.let { name ->
            runCatching { com.spendly.tracker.data.preferences.AppFont.valueOf(name) }
        ...
        preferences.theme.navBarStyle?.let { name ->
            runCatching { com.spendly.tracker.data.preferences.NavBarStyle.valueOf(name) }
        ...
        preferences.theme.coverStyle?.let { name ->
            runCatching { com.spendly.tracker.data.preferences.CoverStyle.valueOf(name) }
