package com.spendly.tracker.data.database
...
import com.spendly.tracker.data.database.entity.BankNotificationEntity
import com.spendly.tracker.data.database.entity.BudgetCategoryEntity
import com.spendly.tracker.data.database.entity.BudgetCategoryMonthSnapshotEntity
import com.spendly.tracker.data.database.entity.BudgetEntity
import com.spendly.tracker.data.database.entity.BudgetMonthSnapshotEntity
import com.spendly.tracker.data.database.entity.CardEntity
...
import com.spendly.tracker.data.database.entity.RuleApplicationEntity
import com.spendly.tracker.data.database.entity.RuleEntity
...
abstract class SpendlyDatabase : RoomDatabase() {
    ...
    companion object {
         ...
         */
        fun getInstance(context: android.content.Context): SpendlyDatabase {
         ...
         */
        fun setInstance(database: SpendlyDatabase) {
            INSTANCE = database
        }
        ...
        val MIGRATION_52_53 = object : Migration(52, 53) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                // Migrate existing secondary category labels (non-primary) to tags
                db.execSQL("""
                    UPDATE `transactions`
                    SET `tags` = (
                        SELECT GROUP_CONCAT(tc.category_name, ',')
                        FROM `transaction_categories` tc
                        WHERE tc.transaction_id = `transactions`.id
                          AND tc.category_name != `transactions`.category
                    )
                    WHERE EXISTS (
        ...
        val MIGRATION_60_61 = object : Migration(60, 61) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_expenses_source_transaction_id` ON `prepaid_expenses` (`source_transaction_id`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_expenses_status` ON `prepaid_expenses` (`status`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_expenses_start_date` ON `prepaid_expenses` (`start_date`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_expenses_end_date` ON `prepaid_expenses` (`end_date`)")

                db.execSQL("""
                    CREATE TABLE IF NOT EXISTS `prepaid_allocations` (
                        `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                        `prepaid_expense_id` INTEGER NOT NULL,
                        `period_year_month` TEXT NOT NULL,
                        `allocated_amount` TEXT NOT NULL,
                        `category` TEXT NOT NULL,
                        `status` TEXT NOT NULL DEFAULT 'PENDING',
                        `recognized_at` TEXT DEFAULT NULL,
                        `created_at` TEXT NOT NULL,
                        `updated_at` TEXT NOT NULL,
                        FOREIGN KEY(`prepaid_expense_id`) REFERENCES `prepaid_expenses`(`id`) ON DELETE CASCADE
                    )
                """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_allocations_prepaid_expense_id` ON `prepaid_allocations` (`prepaid_expense_id`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_allocations_period_year_month` ON `prepaid_allocations` (`period_year_month`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_prepaid_allocations_status` ON `prepaid_allocations` (`status`)")
