...
import com.spendly.shared.data.SharedDataGraph
...
import com.spendly.shared.data.statement.SharedStatementImportResult
import com.spendly.shared.data.util.currentTimeMillis
import com.spendly.shared.data.util.monthStartEpochMillis
import com.spendly.shared.data.util.monthStartEpochMillisIST
import com.spendly.shared.domain.usecase.CreateManualTransactionUseCase
import com.spendly.shared.domain.usecase.ImportStatementUseCase
import com.spendly.shared.domain.usecase.ManualTransactionInput
import com.spendly.shared.domain.usecase.TransactionMutationResult
import com.spendly.shared.domain.usecase.UpdateManualTransactionUseCase
...
class SpendlySharedFacade {
    companion object {
        val shared: SpendlySharedFacade by lazy { SpendlySharedFacade() }
    }
    ...
    fun addExpenseAndLoadHome(
        ...
        addTransactionAndLoadHome(
            ...
            occurredAtEpochMillis = com.spendly.shared.data.util.currentTimeMillis(),
            bankName = null,
            accountLast4 = null
        )
    }
    ...
    fun importStatementTextAndLoadHome(statementText: String): SharedHomeSnapshot = safeSnapshot {
        runBlocking {
            ...
            val debugParser = com.spendly.shared.data.statement.SharedStatementParserFactory.getParser(statementText)
            if (debugParser != null) {
                val debugParsed = debugParser.parse(statementText)
                debugParsed.take(3).forEachIndexed { i, tx ->
                    println("[Spendly DEBUG] Tx #${i + 1}: merchant=${tx.merchant}, amount=${tx.amountMinor}, timestamp=${tx.timestampEpochMillis}, type=${tx.transactionType}, bank=${tx.bankName}")
                }
            }
        ...
        return try {
            runBlocking {
                graph.transactionRepository.softDelete(
                    transactionId,
                    com.spendly.shared.data.util.currentTimeMillis()
                )
                true
            }
        } catch (_: Throwable) {
        ...
        return try {
            runBlocking {
                ...
                graph.categoryRepository.insertCategory(
                    com.spendly.shared.data.model.SharedCategory(
                        name = name,
                        colorHex = colorHex,
                        isSystem = false,
                        isIncome = isIncome,
        ...
        return try {
            runBlocking {
                ...
                graph.accountRepository.insertBalance(
                    com.spendly.shared.data.local.entity.SharedAccountBalanceEntity(
        ...
        return try {
            runBlocking {
                ...
                graph.accountRepository.upsertCard(
                    com.spendly.shared.data.local.entity.SharedCardEntity(
    ...
    fun addSubscriptionAndLoadHome(
        ...
        runBlocking {
            val now = com.spendly.shared.data.util.currentTimeMillis()
        ...
        return try {
            runBlocking {
                ...
                graph.ruleRepository.upsertRule(
                    com.spendly.shared.data.local.entity.SharedRuleEntity(
                        id = id,
                        name = name.trim(),
    ...
    private fun com.spendly.shared.data.local.entity.SharedRuleEntity.toRuleItem() =
        SharedRuleItem(
            id = id,
            name = name,
            conditionsJson = conditionsJson,
    ...
    private fun parseType(value: String): com.spendly.shared.data.model.SharedTransactionType {
        return com.spendly.shared.data.model.SharedTransactionType.entries.firstOrNull {
            it.name.equals(value, ignoreCase = true)
        } ?: com.spendly.shared.data.model.SharedTransactionType.EXPENSE
    }
    ...
    private fun com.spendly.shared.data.model.SharedTransaction.toItem() =
    ...
    private fun com.spendly.shared.data.local.entity.SharedAccountBalanceEntity.toAccountItem() =
        SharedAccountItem(
            bankName = bankName,
            accountLast4 = accountLast4,
            balanceMinor = balanceMinor,
    ...
    private fun com.spendly.shared.data.local.entity.SharedCardEntity.toCardItem() =
        SharedCardItem(
            id = id,
            cardLast4 = cardLast4,
            cardType = cardType,
    ...
    private fun com.spendly.shared.data.model.SharedCategory.toCategoryItem() =
        SharedCategoryItem(
            id = id,
            name = name,
            colorHex = colorHex,
